...
def writeSetupRepeatingBlocksImplementation(fileDescriptor, className):
    ...
    writeLine(fileDescriptor, '    }')
    writeLine(fileDescriptor)
    writeLine(fileDescriptor, '    // Header has length 2')
    writeLine(fileDescriptor, '    const auto repeatingBlocksDataSize = m_blockData.size() - 2 - m_fixedBlockLength;')
    ...
    writeLine(fileDescriptor, '    const auto repeatingBlocksOffset = m_fixedBlockLength + 2;')
