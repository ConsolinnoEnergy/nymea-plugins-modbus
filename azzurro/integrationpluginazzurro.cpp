...
{
    m_powerControl = new PowerControlAzzurro();
}
...
void IntegrationPluginAzzurro::executeAction(ThingActionInfo *info)
{
    Thing *thing = info->thing();

    if (thing->thingClassId() == azzurroInverterRTUThingClassId)
    {
        AzzurroModbusRtuConnection *azzurromodbusrtuconnection = m_rtuConnections.value(thing);
        bool success = false;
        ActionTypeId actionTypeId = info->action().actionTypeId();

        if (!azzurromodbusrtuconnection)
        {
            qCWarning(dcAzzurro()) << "Modbus connection not available";
        ...
        if (!success)
        {
            qCWarning(dcAzzurro()) << "Action execution finished with error.";
            info->finish(Thing::ThingErrorHardwareFailure);
            return;
        }
...
bool IntegrationPluginAzzurro::executePowerControl(AzzurroModbusRtuConnection *azzurromodbusrtuconnection)
{
    ...
    ModbusRtuReply *reply = azzurromodbusrtuconnection->writeBlockPowerControl(values);
    return handleReply(reply);
}

bool IntegrationPluginAzzurro::handleReply(ModbusRtuReply *reply)
{
    if (!reply)
    {
        qCWarning(dcAzzurro()) << "Sending modbus command failed because the reply could not be created.";
        // m_errorOccured = true;
        return false;
    }
    ...
    connect(reply, &ModbusRtuReply::errorOccurred, this, [reply](ModbusRtuReply::Error error)
            {
                qCWarning(dcAzzurro()) << "Modbus reply error occurred while writing modbus" << error << reply->errorString();
