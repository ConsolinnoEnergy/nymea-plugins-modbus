...
bool SgReadyInterface::setSgReadyMode(SgReadyMode sgReadyMode)
{
    ...
    if (!m_gpio1->setValue(gpioSettings.first ? Gpio::ValueHigh : Gpio::ValueLow)) {
        qCWarning(dcLambda()) << "Could not switch GPIO 1 for setting" << sgReadyMode;
        return false;
    }

    if (!m_gpio2->setValue(gpioSettings.second ? Gpio::ValueHigh : Gpio::ValueLow)) {
        qCWarning(dcLambda()) << "Could not switch GPIO 2 for setting" << sgReadyMode;
        return false;
    }
...
bool SgReadyInterface::setup(bool gpio1Enabled, bool gpio2Enabled)
{
    ...
    if (!m_gpio1) {
        qCWarning(dcLambda()) << "Failed to set up SG ready interface gpio 1" << m_gpioNumber1;
        return false;
    }
    ...
    if (!m_gpio2) {
        delete m_gpio1;
        m_gpio1 = nullptr;
        qCWarning(dcLambda()) << "Failed to set up SG ready interface gpio 2" << m_gpioNumber1;
        return false;
    }
