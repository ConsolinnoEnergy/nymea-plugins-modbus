...
void SolaxEvcModbusTcpConnection::update6()
{
    ...
    qCDebug(dcSolaxEvcModbusTcpConnection())
            << "--> Read block \"meterValues\" registers from:" << 4096 << "size:" << 18;
    if (!reply) {
        qCWarning(dcSolaxEvcModbusTcpConnection())
    ...
    connect(reply, &QModbusReply::finished, this, [this, reply]() {
        ...
        qCDebug(dcSolaxEvcModbusTcpConnection()) << "<-- Response from reading block \"meterValues\" register" << 4096 << "size:" << 18 << blockValues;
        ...
        processTotalEnergyRegisterValues(blockValues.mid(16, 2));
        update7();
    });
...
void SolaxEvcModbusTcpConnection::updateMeterValuesBlock()
{
    ...
    qCDebug(dcSolaxEvcModbusTcpConnection()) << "--> Read block \"meterValues\" registers from:" << 4096 << "size:" << 18;
...
{
    QModbusDataUnit request = QModbusDataUnit(QModbusDataUnit::RegisterType::InputRegisters, 4112, 2);
    return sendReadRequest(request, m_slaveId);
}
...
{
    QModbusDataUnit request = QModbusDataUnit(QModbusDataUnit::RegisterType::InputRegisters, 4096, 18);
    return sendReadRequest(request, m_slaveId);
}
...
{
    float receivedTotalEnergy = ModbusDataUtils::convertToUInt32(values, ModbusDataUtils::ByteOrderBigEndian) * 1.0 * pow(10, -1);
