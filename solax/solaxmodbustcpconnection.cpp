...
QModbusReply *SolaxModbusTcpConnection::setUnlockPassword(quint16 unlockPassword)
{
    QVector<quint16> values = ModbusDataUtils::convertFromUInt16(unlockPassword);
    qCDebug(dcSolaxModbusTcpConnection()) << "--> Write \"Unlock password (0x00)\" register:" << 0 << "size:" << 1 << values;
...
QModbusReply *SolaxModbusTcpConnection::setModeType(quint32 modeType)
{
    ...
    qCDebug(dcSolaxModbusTcpConnection()) << "--> Write \"Control mode and target type (0x7C)\" register:" << 124 << "size:" << 2 << values;
    QModbusDataUnit request = QModbusDataUnit(QModbusDataUnit::RegisterType::HoldingRegisters, 124, values.count());
    request.setValues(values);
    return sendWriteRequest(request, m_slaveId);
}

QModbusReply *SolaxModbusTcpConnection::setForceBatteryPower(qint32 forceBatteryPower)
{
    QVector<quint16> values = ModbusDataUtils::convertFromInt32(forceBatteryPower, m_endianness);
    qCDebug(dcSolaxModbusTcpConnection()) << "--> Write \"Battery power (0x89)\" register:" << 137 << "size:" << 2 << values;
...
void SolaxModbusTcpConnection::initialize3()
{
    ...
    // Read identification
    qCDebug(dcSolaxModbusTcpConnection()) << "--> Read init block \"identification\" registers from:" << 7 << "size:" << 14;
...
void SolaxModbusTcpConnection::update6()
{
    ...
    qCDebug(dcSolaxModbusTcpConnection()) << "--> Read \"Read grid export limit (0xB6)\" register:" << 182 << "size:" << 1;
    reply = readReadExportLimit();
    if (!reply) {
        qCWarning(dcSolaxModbusTcpConnection()) << "Error occurred while reading \"Read grid export limit (0xB6)\" registers from" << hostAddress().toString() << errorString();
        return;
    }
    ...
    connect(reply, &QModbusReply::finished, this, [this, reply](){
        ...
        const QModbusDataUnit unit = reply->result();
        qCDebug(dcSolaxModbusTcpConnection()) << "<-- Response from \"Read grid export limit (0xB6)\" register" << 182 << "size:" << 1 << unit.values();
        processReadExportLimitRegisterValues(unit.values());
        update7();
    });

    connect(reply, &QModbusReply::errorOccurred, this, [this, reply] (QModbusDevice::Error error){
        qCWarning(dcSolaxModbusTcpConnection()) << "Modbus reply error occurred while reading \"Read grid export limit (0xB6)\" registers from" << hostAddress().toString() << error << reply->errorString();
    });
}
...
void SolaxModbusTcpConnection::updateReadExportLimit()
{
...
void SolaxModbusTcpConnection::updateIdentificationBlock()
{
    ...
    qCDebug(dcSolaxModbusTcpConnection()) << "--> Read block \"identification\" registers from:" << 7 << "size:" << 14;
    ...
    connect(reply, &QModbusReply::finished, this, [this, reply](){
        ...
        if (reply->error() == QModbusDevice::NoError) {
            const QModbusDataUnit unit = reply->result();
            const QVector<quint16> blockValues = unit.values();
            qCDebug(dcSolaxModbusTcpConnection()) << "<-- Response from reading block \"identification\" register" << 7 << "size:" << 14 << blockValues;
            processFactoryNameRegisterValues(blockValues.mid(0, 7));
            processModuleNameRegisterValues(blockValues.mid(7, 7));
        }
    });
...
QModbusReply *SolaxModbusTcpConnection::readReadExportLimit()
{
