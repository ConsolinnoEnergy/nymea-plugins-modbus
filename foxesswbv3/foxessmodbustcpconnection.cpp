...
* ADDITION - Luis Reber
*
* Modifications have been made to this file (lines 130f).
* If changes are made to the register json file, make sure a new file is generated.
* The changes have to be merged into this file.
*
*
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
{
    // FoxESS WB does not allow write with function code 0x6, only with 0x10 -> Workaround needed
    // Force nymea to use 0x10 by writing two registers, this overwrites maxChargePower
    // The following code makes sure that maxChargePower stays consistent
    ...
    quint32 combinedValue = (maxCurrent << 16) | maxPower;
    qCWarning(dcFoxESSModbusTcpConnection()) << "#### Setting max current to" << combinedValue;
    qCWarning(dcFoxESSModbusTcpConnection()) << "MaxCurrent to" << maxCurrent;
    qCWarning(dcFoxESSModbusTcpConnection()) << "MaxPower to" << maxPower;
